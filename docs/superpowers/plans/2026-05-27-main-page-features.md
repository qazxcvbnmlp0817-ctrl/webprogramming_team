...
- [ ] **Step 1: 테스트 파일의 모크 데이터를 업데이트하고 필터 버튼 렌더링 테스트 추가**
...
- [ ] **Step 1: MainPage.tsx 전체를 아래 내용으로 교체**
...
- [ ] DB에 게시글이 없을 때 더미 데이터가 좋아요 순으로 표시된다
- [ ] 필터 '학사' 선택 → '학사' 카테고리 공지만 표시된다
- [ ] 필터 선택 → 로그아웃 → 재로그인 후 같은 사용자명으로 접속 시 필터가 유지된다
- [ ] 공지 아이템 클릭 → `/notice/:id` 상세 페이지로 이동한다
- [ ] 게시글 아이템 클릭 → `/post/:id` 상세 페이지로 이동한다
- [ ] '더보기 →' 링크는 여전히 목록 페이지(`/dept/notice`, `/dept/board`)로 이동한다
