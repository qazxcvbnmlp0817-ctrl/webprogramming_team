...
import { useSchoolEdit } from '../../../context/SchoolEditContext'
...
interface Props {
  ...
  renderForm: (value: SchoolPageContentDto, onChange: (v: SchoolPageContentDto) => void) => ReactNode
  children: ReactNode
}

export default function SchoolEditableSection({
  sectionKey,
  title,
  value,
  renderForm,
  ...
  const { isEditMode, saveSection, saving } = useSchoolEdit()
  const [open, setOpen] = useState(false)
  const [draft, setDraft] = useState<SchoolPageContentDto>(value)
  ...
  return (
    <div className="relative outline outline-2 outline-blue-400 outline-offset-[-2px]">
      {children}
      <button
        type="button"
        onClick={handleOpen}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white border-2 border-black w-full max-w-2xl max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between p-4 border-b-2 border-black">
              ...
              <button type="button" onClick={() => setOpen(false)} className="text-gray-500 hover:text-black">
            ...
            {error && <p className="px-5 pb-2 text-red-600 text-sm font-bold">{error}</p>}
