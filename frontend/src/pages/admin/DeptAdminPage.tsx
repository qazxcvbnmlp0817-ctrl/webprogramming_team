...
import { DeptEditProvider } from '../../context/DeptEditContext'
...
type Tab = '개요' | '학과 페이지' | '게시글 관리' | '공지 관리' | '사용자' | '가입 승인' | '통계' | '교수 배정'
const TABS: Tab[] = ['개요', '학과 페이지', '게시글 관리', '공지 관리', '사용자', '가입 승인', '통계', '교수 배정']

export default function DeptAdminPage() {
  const navigate = useNavigate()
  const { id } = useParams<{ id: string }>()
  const deptCtx = useDept()
  ...
  const [univProfessors, setUnivProfessors] = useState<ProfessorItem[]>([])
  ...
  useEffect(() => {
    fetchDeptPosts(postPage, deptId).then(d => {
      setPosts(d.posts); setPostTotalPages(d.totalPages)
    })
  }, [postPage, deptId])

  useEffect(() => {
    fetchDeptNotices(noticePage, deptId).then(d => {
  ...
  const handleStatusChange = async (userId: number, newStatus: string) => {
    ...
    fetchDeptPendingUsers(deptId).then(setPending)
  }
  ...
  const gotoWrite = (path: string) => {
    if (id) deptCtx.setDept({
      selectedDeptId: Number(id),
      selectedDeptName: deptCtx.selectedDeptName,
      selectedUniversityId: deptCtx.selectedUniversityId,
      selectedUniversityName: deptCtx.selectedUniversityName,
      selectedSchoolName: deptCtx.selectedSchoolName,
    })
    navigate(path)
  }
  ...
  return (
    <div className="bg-white text-black font-sans min-h-screen">
      ...
      <section className="bg-black text-white py-10 px-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div>
              <p className="text-xs text-gray-400 tracking-widest uppercase mb-1">Dept Admin</p>
              <h1 className="text-2xl font-bold">학과 관리자 대시보드</h1>
              <p className="text-gray-400 text-sm mt-1">학과 ID: {id}</p>
            </div>
            {isPrivileged && (
          ...
          <div className="flex gap-2">
            <button onClick={() => gotoWrite('/dept/board/write')}
              className="border border-gray-400 text-white px-3 py-2 text-xs hover:bg-white hover:text-black transition">
              <i className="fas fa-pen mr-2" />학과 글쓰기
            </button>
            <button onClick={() => gotoWrite('/dept/notice/write')}
              className="border border-gray-400 text-white px-3 py-2 text-xs hover:bg-white hover:text-black transition">
      ...
      {tab === '학과 페이지' ? (
        <DeptEditProvider deptId={deptId} onSaved={() => setRefreshKey(k => k + 1)}>
          <DepartmentPage key={refreshKey} embedded deptIdOverride={deptId} />
        </DeptEditProvider>
      ) : (
      <main className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        ...
        {tab === '사용자' && (
          <div className="border-2 border-black p-6">
            <h2 className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-4">학과 사용자</h2>
              ...
              <div className="flex flex-wrap gap-3 items-end">
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    <button
                      onClick={async () => {
                        if (!showExtProf && univProfessors.length === 0) {
                          const all = await fetchDeptUnivProfessors(deptId)
                          setUnivProfessors(all)
                        }
                        setShowExtProf(v => !v)
                        setExtProfSearch('')
                      }}
                  ...
                  {showExtProf && (
                    <div className="mt-2 border border-gray-300 rounded p-3 w-80 bg-white shadow-sm">
                      <p className="text-xs text-gray-500 mb-2">학교 전체 교수 검색</p>
                      <input
                        value={extProfSearch}
                      ...
                      <div className="max-h-48 overflow-y-auto flex flex-col gap-0.5">
                        ...
                        {univProfessors.filter(p => !extProfSearch || p.name.includes(extProfSearch)).length === 0 && (
  ...
  return (
    <table className="w-full text-sm">
      ...
      <tbody>
        ...
        {users.length === 0 && (
          <tr><td colSpan={6} className="py-8 text-center text-gray-400 text-sm">학과 소속 사용자가 없습니다.</td></tr>
        )}
      </tbody>
    </table>
  )
