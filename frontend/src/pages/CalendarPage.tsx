...
function periodToTime(period: number): string {
  const minutes = 510 + period * 30  // period 1 → 540 = 09:00
...
// 시간표 → 날짜별 ScheduleItem (±3개월, readonly) — LectureOffering 기반
...
export default function CalendarPage() {
  ...
  // ── 시간표 로드 (LectureOffering 기반, readonly 반복 수업) ────────────────
  // 학생이 실제로 담은 분반(LectureOffering)의 시간만 표시. ClassScheduleDto 경로는
  // courseId 매칭으로 다른 교수 분반까지 끌어오는 한계가 있어 사용하지 않음.
  useEffect(() => {
    if (!loggedIn || !username) return
  ...
  // ── 전체 일정 (통합 DB + ClassEvent + 시간표 병합, id 중복 제거) ──
  const allSchedules = useMemo<ScheduleItem[]>(() => {
    const base = [...dbSchedules, ...timetableItems]
