            ...
            <div className="flex-1">
              <div className="border-2 border-black">
                ...
                <div className="grid grid-cols-7 border-b border-gray-200">
                  {['일', '월', '화', '수', '목', '금', '토'].map((d, i) => (
                    <div key={d} className={`text-center py-2 text-sm font-bold ${i === 0 ? 'text-red-500' : i === 6 ? 'text-blue-500' : 'text-gray-700'}`}>
                      {d}
                    </div>
                  ))}
                </div>
                      ...
                      <div key={day} className={`border-b border-r border-gray-100 min-h-[80px] p-1 ${isToday ? 'bg-gray-50' : ''}`}>
                        ...
                        {daySchedules.slice(0, 2).map(s => (
                          <div key={s.id} className={`text-xs px-1 py-0.5 rounded mb-0.5 truncate font-medium
                            ${s.category === '시험' ? 'bg-red-100 text-red-700' :
                              s.category === '학사' ? 'bg-blue-100 text-blue-700' :
                              s.category === '행사' ? 'bg-green-100 text-green-700' :
                              'bg-gray-100 text-gray-700'}`}>
                            {s.title}
                          </div>
                        ))}
                        {daySchedules.length > 2 && (
                          <div className="text-xs text-gray-400 px-1">+{daySchedules.length - 2}개</div>
                        )}
                      </div>
                    )
                  })}
              ...
              <div className="mt-6 border-2 border-black p-4">
                <h3 className="font-bold mb-3 flex items-center gap-2">
                  ...
                  오늘의 일정 ({today.getMonth() + 1}/{today.getDate()})
                </h3>
